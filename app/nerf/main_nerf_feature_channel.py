...
def generate_and_save_feature_images(trainer):
    ...
    log.info("Beginning rendering...")
    img_shape = trainer.validation_dataset.img_shape
    log.info(f"Running rendering on dataset with {len(trainer.validation_dataset)} images "
             f"at resolution {img_shape[0]}x{img_shape[1]}")

    trainer.valid_log_dir = os.path.join(trainer.log_dir, "feature_2d_renders")
    log.info(f"Saving rendering result to {trainer.valid_log_dir}")
