...
from wisp.datasets import MultiviewDataset, SampleRays, SampleRaysWithSparseChannel, SampleRaysCalculateCorrespondences
...
from wisp.trainers.multiview_trainer import MultiviewWithSparseDepthGtTrainer, MultiviewWithDiftTrainer
...
def parse_args():
    ...
    data_group.add_argument('--dift-scene-name', type=str,
                            help='')
    data_group.add_argument('--dift-img-size', type=int, default=256,
                            help='')
    data_group.add_argument('--dift-ensemble', type=int, default=4,
                                help='')
    ...
    optimizer_group.add_argument('--consistent-loss-lambda', type=float, default=1.0,
                                 help='Weight of consistent rgb loss')
...
def load_dataset(args) -> MultiviewDataset:
    ...
    """
    transform = SampleRaysCalculateCorrespondences(num_samples=args.num_rays_sampled_per_img)
    train_dataset = wisp.datasets.NeRFSyntheticDatasetWithDIFT(dataset_path=args.dataset_path,
                                                         split='train',
                                                         bg_color=args.bg_color,
                                                         scene_name=args.dift_scene_name,
                                                         dift_img_size=args.dift_img_size,
                                                         dift_ensemble=args.dift_ensemble,
                                                         mip=args.mip,
...
def load_trainer(pipeline, train_dataset, validation_dataset, device, scene_state, args, args_dict) -> BaseTrainer:
    ...
    trainer = MultiviewWithDiftTrainer(pipeline=pipeline,
...
if __name__ == '__main__':
    insert_args_to_cli = [
        ...
        '--dift-scene-name', 'a fern plant in the lobby of a fancy building',
        '--dift-img-size', '256',
        '--dift-ensemble', '4',
