...
if __name__ == '__main__':
    insert_args_to_cli = [
        ...
        '--pretrained', '_results/logs/runs/reg_prune_no_depthloss/20230425-193833/model.pth'
    ]
    ...
    pipeline_with_depthloss = torch.load('_results/logs/runs/reg_prune_with_depthloss/20230425-194307/model.pth')
    pipeline_no_depthloss = torch.load('_results/logs/runs/reg_prune_no_depthloss/20230425-193833/model.pth')
    scene_state = WispState()   # Joint trainer / app state
    trainer = load_trainer(pipeline=pipeline_with_depthloss,
    ...
    pips_model = None
    dataset = trainer.validation_dataset
    lod_idx = lods[-1]
    name = f"lod{lods[-1]}"
    ...
    with torch.no_grad():
        for pipeline, pipeline_name in [(pipeline_with_depthloss, 'with depthloss'), (pipeline_no_depthloss, 'no depthloss')]:
            for idx in tqdm(range(len(dataset))):
                ...
                rb = trainer.renderer.render(pipeline, rays, lod_idx=lod_idx)
                ...
                in_rb = RenderBuffer(rgb=gts, gts=gts, err=(gts[..., :3] - gts[..., :3]) ** 2)

                cv2.imshow(f'image {idx} train {pipeline_name}', out_rb.cpu().image().byte().rgb.numpy())
                cv2.waitKey(30)
        ret_key = None
        while ret_key != ord('q'):
